...
//this function is exported and called in views, when the route is right
export const AnimalForm = () => {
    ...
    //this says look at variable within the route
    ...
    useEffect(() => {
      getCustomers().then(getLocations).then(getSpecies).then(() => {
        if (animalId){
          ...
          .then(animal => {
              setAnimal(animal)
              //if this is true, you cant click the save button
              setIsLoading(false)
          })
        } else {
          setIsLoading(false)
